    ...
    public void calculateBill() {
        if (customerType == 1) {
            if (gallonsUsed <= SINGLE_TIER1) {
                ...
                bill = SINGLE_BASE + SINGLE_TIER1 * (SINGLE_TIER1_COST / GALLONS) + (gallonsUsed - SINGLE_TIER1) * (SINGLE_TIER2_COST / GALLONS);
            } else {
                bill = SINGLE_BASE + SINGLE_TIER1 * (SINGLE_TIER1_COST / GALLONS) + SINGLE_TIER2 * (SINGLE_TIER2_COST / GALLONS) + (gallonsUsed - TIER2_CUTOFF) * (SINGLE_TIER3_COST / GALLONS);
            }
        } else {
            if (gallonsUsed <= DUPLEX_TIER1) {
                bill = DUPLEX_BASE + gallonsUsed * (DUPLEX_TIER1_COST / GALLONS);
            } else if (gallonsUsed <= TIER2_CUTOFF) {
                bill = DUPLEX_BASE + DUPLEX_TIER1 * (DUPLEX_TIER1_COST / GALLONS) + (gallonsUsed - DUPLEX_TIER1) * (DUPLEX_TIER2_COST / GALLONS);
            } else {
                bill = DUPLEX_BASE + DUPLEX_TIER1 * (DUPLEX_TIER1_COST / GALLONS) + DUPLEX_TIER2 * (DUPLEX_TIER2_COST / GALLONS) + (gallonsUsed - TIER2_CUTOFF) * (DUPLEX_TIER3_COST / GALLONS);
            }
        }
    }
}
